...
extension HistoryViewController: UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let celebrityLabel = cell.viewWithTag(2) as? UILabel{
            celebrityLabel.text = DataManager.shared.recordList[indexPath.row].celebrityValue ?? "없음"
        }
        if let ageLabel = cell.viewWithTag(3) as? UILabel{
            ageLabel.text = DataManager.shared.recordList[indexPath.row].faceAgeValue ?? "없음"
        }
